<template>
  <my-page class="page-reader" :title="title" :page="page" :style="pageStyle">
    ...
    >
      ...
      <ul class="note-list" v-if="notes.length">
        ...
        >
          ...
          <div class="mark" :style="{ 'border-color': note.color }">
            {{ note.selectedText }}
          </div>
    ...
    <div class="page-info" v-if="info">
      {{ info.page }} / {{ info.totalPage }}
      {{ (info.page / info.totalPage) * 100 }}%
    ...
    <div class="select-menu" id="select-menu">
      ...
      <div class="menu-item" title="使用百度搜索" @click="searchNetwork">
        搜索
      </div>
    </div>
  </my-page>
...
export default {
  ...
  methods: {
          ...
          resultPromises.push(
            new Promise((resolve, reject) => {
              new Promise(function(resolve, reject) {
                resolve(
                  new EPUBJS.Chapter(spineItem, book.store, book.credentials)
                );
              })
    ...
    next() {
      ...
      this.$axios
        .post(this.GLOBAL.host + "/pushProgress", {
          progress: this.locationCfi,
          user_id: "1",
          publisher_content_id: this.bookId
        })
        .then(res => {
    ...
    addBookmark() {
      let name =
        "书签 " + new Date().getHours() + ":" + new Date().getMinutes();
    ...
    loadBook(content) {
      ...
      this.book.on("renderer:mouseup", event => {
        ...
        console.log(
          this.selection.anchorNode.data.substring(
            this.selection.baseOffset,
            this.selection.extentOffset
          )
        );
        this.selectedText = this.selection.anchorNode.data.substring(
          this.selection.baseOffset,
          this.selection.extentOffset
        );
      ...
      this.$axios
        ...
        .then(res => {
          ...
          this.book.goto(res.data.result.progress);
        });
    },
    setStyle() {
      // TODO ???
